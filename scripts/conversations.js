...
async function GenerateConversation(OtherUsername, Between, ConversationDoc) {
    ...
    Conversation.appendChild(ConversationCoverImage);

    const Members = Between.map(user => user.replace(/"/g, ''));
    const MembersData = await Promise.all(
        Members.map(async (Username) => {
            const MemberQuery = await query(UsersCollection, where("username", "==", Username));
            const MemberSnapshot = await getDocs(MemberQuery);
            return MemberSnapshot.docs[0]?.data() || null;
        })
    );

    const ConversationName = ConversationDoc.data().name ? ConversationDoc.data().name : OtherUsername;
    ...
    if (IsGroup) {
        ...
        const Loop = async () => {
            ConversationCoverImage.src = MembersData[Index]?.profilePhoto !== "" 
                ? MembersData[Index]?.profilePhoto 
