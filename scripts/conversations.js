...
async function GenerateConversation(OtherUsername, Between, ConversationDoc) {
    ...
    if (ConversationDoc.id === localStorage.getItem("LastConversationId")) {
        Conversation.style.order = "-1";
    }
    ...
    Conversation.onclick = async () => {
        ...
        const NewConversationDoc = await getDoc(doc(Db, "conversations", ConversationDoc.id));
        GenerateMessages(NewConversationDoc.id, NewConversationDoc.data().messages);

        localStorage.setItem("LastConversationId", ConversationDoc.id);

        Array.from(ConversationsContainer.children).forEach(OtherConversation => {
            if (OtherConversation.tagName === "div" && OtherConversation !== Conversation) {
                OtherConversation.style.order = "";
                Conversation.style.order = "-1";
            }
        });        
    };
...
function Loop() {
    Api.AddVideoControls();
    requestAnimationFrame(Loop);
}

ImageUploadButton.addEventListener("click", async () => {
    const Media = ImageInput.files[0];
    if (!Media) return;

    ImageUploadButton.innerHTML = "Uploading";
    ImageUploadButton.disabled = true;

    const StorageRef = ref(getStorage(), `uploads/${Media.name}`);
    ...
    if (Api.ImageExtensions.includes(Extension)) {
        MessageInput.value += ` <img>${MediaURL}</img>`;
        ImagePrompt.style.visibility = 'hidden';
    } else if (Api.VideoExtensions.includes(Extension)) {
        MessageInput.value += ` <video>${MediaURL}</video>`;
        ImagePrompt.style.visibility = 'hidden';
    } else if (Api.AudioExtensions.includes(Extension)) {
        MessageInput.value += ` <audio>${MediaURL}</audio>`;
        ImagePrompt.style.visibility = 'hidden';
    }
});
