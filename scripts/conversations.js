...
async function GenerateConversation(OtherUsername, Between, ConversationDoc) {
    ...
    const ConversationStatus = document.createElement("img");
    ...
    if (IsGroup) {
        ...
        MembersData.forEach(MemberData => {
            if (MemberData && MemberData.status && MemberData.status.state === "Online") {
                ConversationStatus.src = "../images/Online.svg";
                OnlineCount++;
            } else {
                ConversationStatus.src = "../images/Offline.svg";
            }
        });
    
        ConversationStatus.src = OnlineCount === Members.length ? "../images/Online.svg" : "../images/Offline.svg";
        ...
        ConversationStatus.src = OtherUserData.status ? OtherUserData.status.state === "Online" ? "../images/Online.svg" : "../images/Offline.svg" : "../images/Offline.svg";
        ConversationCoverImage.src = OtherUserData.profilePhoto || "../images/DefaultUser.svg";
    }

    const ConversationNameLabel = document.createElement("input");
    ConversationNameLabel.value = ConversationName;
    ConversationNameLabel.readOnly = window.MobileCheck() ? true : IsGroup ? false : true;
    ConversationNameLabel.onkeypress = async (Event) => {
        if (Event.key === "Enter" && ConversationNameLabel.value && ConversationNameLabel.value !== ConversationName) {
            await updateDoc(ConversationDoc.ref, {
                name: ConversationNameLabel.value,
            });
            location.reload();
        }
    };
    ...
    Conversation.onclick = (Event) => {
        if (Event.target !== Conversation) return;
